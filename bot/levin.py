...
def log_line(prefix: str, message: str) -> None:
    """
    Append one line to the log in the format:
      <prefix>: <message>
    Where prefix is typically 'JUDGE' for incoming lines and 'Agent' for our replies.
    """
    try:
        ...
        # Logging should never crash the bot; report to stderr only.
...
def send_agent_move(dx: int, dy: int) -> None:
    """
    Print the move to stdout (so the judge can read it) and log it as Agent: <dx> <dy>.
    """
    out = f"{dx} {dy}"
    print(out)
    log_agent(out)
...
def read_observation(old_state: State) -> Optional[State]:
    ...
    # We read exactly (2R+1) lines; log each original line from the judge
    for i in range(2 * circuit_data.visibility_radius + 1):
        ...
        visible_track[x, y_start:y_end] = line_vals
...
def main():
    # Optionally, mark session start in the log
    log_line("INFO", "Session start")
    ...
    while True:
        ...
        if state is None:
            # we also note the end in the log
            log_line("INFO", "Judge signaled end (~~~END~~~)")
            return
        dx, dy = calculate_move(rng, state)
        send_agent_move(dx, dy)
